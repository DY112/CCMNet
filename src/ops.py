...
def uv_histogram_to_rgb_image(uv_hist, y_value=1, contrast_scale=1, save_path=None, boundary=None):
    """
    Converts a UV histogram to an RGB image and saves it.

    Args:
        uv_hist (np.ndarray): UV histogram.
        y_value (float): Brightness value for conversion to RGB.
        contrast_scale (float): Scaling factor to adjust contrast.
        save_path: Path to save the generated image.
    """
    assert len(uv_hist.shape) == 2 and uv_hist.shape[0] == uv_hist.shape[1]
    hist_size = uv_hist.shape[0]

    # print(uv_hist.min(), uv_hist.max())
    # input()
    uv_hist = (uv_hist - np.min(uv_hist)) / (np.max(uv_hist) - np.min(uv_hist))
    # uv_hist = np.power(uv_hist, 0.8)

    if boundary == None:
        u_coord, v_coord = get_uv_coord(hist_size, tensor=False, normalize=False)
        U, V = from_coord_to_uv(hist_size, u_coord, v_coord)
        uv = np.stack([U, V], axis=-1).reshape(-1, 2)
    else:
        start = boundary[0] + ((boundary[1] - boundary[0]) / hist_size) / 2
        end = boundary[1] - ((boundary[1] - boundary[0]) / hist_size) / 2
        U, V = np.meshgrid(
            np.linspace(start, end, hist_size),
            np.linspace(start, end, hist_size)
            )
        uv = np.stack([U, V], axis=-1).reshape(-1, 2)
    rgb_colors = uv_to_rgb(uv) * y_value

    rgb_image = (rgb_colors.reshape(hist_size, hist_size, 3) * uv_hist[:, :, None])
    rgb_image = np.clip(rgb_image * contrast_scale, 0, 1)

    plt.figure(figsize=(4, 4))
    plt.axis('off')
    plt.imshow(rgb_image, interpolation='bicubic')
    plt.axhline(y=hist_size / 2, color='white', linewidth=0.5)
    plt.axvline(x=hist_size / 2, color='white', linewidth=0.5)
    
    if save_path is not None:
        plt.savefig(save_path, bbox_inches='tight', pad_inches=0, dpi=300)
    else:
        save_path = 'uv_histogram.png'
        plt.savefig(save_path, bbox_inches='tight', pad_inches=0, dpi=300)
    plt.close()
...
def get_current_datetime():
  """ Returns the current date and time in YYMMDD-HHMMSS format. """
